...
#define WINDOW_WIDTH    730   // size of window
...
int playerHp, playerMp, playerLevel, playerExp, playerSp, playerSkillS, playerSkillD;
char id[NAMELEN];
BOB			 npc[MAX_NPC + MAX_OBSTACLE];      // NPC Unit
...
#define CHARACTER_TEXTURE 0
#define MONSTER_TEXTURE  1
#define OBSTACLE_TEXTURE 2
SOCKET g_mysocket;
WSABUF	send_wsabuf;
...
void ProcessPacket(char *ptr)
{
	...
	case SC_PACKET_PUT_PLAYER :
	{
		...
		if (id == g_myid) {
			...
			playerHp = my_packet->hp;
			playerMp = my_packet->mp;
			playerLevel = my_packet->level;
			playerExp = my_packet->exp;
			playerSp = my_packet->sp;
	...
	{
		sc_packet_update_player *my_packet = reinterpret_cast<sc_packet_update_player*>(ptr);
		switch (my_packet->updateType) {
		case LEVELUP:
			++playerLevel;
			playerExp = 0;
			break;
	...
	LPARAM lparam)
{
	...
	{
	case WM_KEYDOWN: {
		int x = 0, y = 0, a = 0, s = 0, d = 0;
		if (wparam == VK_RIGHT)	x += 1;
		if (wparam == VK_LEFT)	x -= 1;
		if (wparam == VK_UP)	y -= 1;
		if (wparam == VK_DOWN)	y += 1;

		if (wparam == 'a' || wparam == 'A') a = 1;
		if (wparam == 's' || wparam == 'S') s = 1;
		if (wparam == 'd' || wparam == 'D') d = 1;
		...
		if (0 != a) {
			my_packet->type = CS_PACKET_ATTACK;
			WSASend(g_mysocket, &send_wsabuf, 1, &iobyte, 0, NULL, NULL);
		}
...
int Game_Init(void *parms)
{
	...
	Load_Image_Bitmap32(&black_tile, L"assets/grass.BMP", 0, 0, BITMAP_EXTRACT_MODE_ABS);
	white_tile.x = 0;
	white_tile.y = 0;
	white_tile.height = TILE_WIDTH;
	white_tile.width = TILE_WIDTH;
	Load_Image_Bitmap32(&white_tile, L"assets/grass2.BMP", 0, 0, BITMAP_EXTRACT_MODE_ABS);

	// now let's load in all the frames for the skelaton!!!

	// 이미지 크기
	// Character / Monster Image load
	Load_Texture(L"assets/newKnight.png", CHARACTER_TEXTURE, 315, 105);
	Load_Texture(L"assets/newOrc.png", MONSTER_TEXTURE, 315, 105);
	Load_Texture(L"assets/Rock.png", OBSTACLE_TEXTURE, 35, 35);

	// Player의 크기는 64x64 크기
	//if (!Create_BOB32(&player, 0, 0, 64, 64, 1, BOB_ATTR_SINGLE_FRAME))
	if (!Create_BOB32(&player, 0, 0, 35, 35, 1, BOB_ATTR_SINGLE_FRAME))
		return(0);
	// 0~ index중 (2 , 0)번째 index를 불러온다.
	//Load_Frame_BOB32(&player, UNIT_TEXTURE, 0, 2, 0, BITMAP_EXTRACT_MODE_CELL);
	Load_Frame_BOB32(&player, CHARACTER_TEXTURE, 0, 4, 1, BITMAP_EXTRACT_MODE_CELL);
	...
	for (int i = 0; i < MAX_USER; ++i) {
		if (!Create_BOB32(&skelaton[i], 0, 0, 35, 35, 1, BOB_ATTR_SINGLE_FRAME))
			return(0);
		// 0~ index중 (0 , 0)번째 index를 불러온다.
		Load_Frame_BOB32(&skelaton[i], CHARACTER_TEXTURE, 0, 4, 1, BITMAP_EXTRACT_MODE_CELL);
	...
	for (int i = 0; i < MAX_NPC; ++i) {
		if (!Create_BOB32(&npc[i], 0, 0, 35, 35, 1, BOB_ATTR_SINGLE_FRAME))
			return(0);
		// 4, 0 번째 index 불러온다.
		Load_Frame_BOB32(&npc[i], MONSTER_TEXTURE, 0, 3, 2, BITMAP_EXTRACT_MODE_CELL);
	...
	for (int i = MAX_NPC; i < MAX_NPC + MAX_OBSTACLE; ++i) {
		if (!Create_BOB32(&npc[i], 0, 0, 35, 35, 1, BOB_ATTR_SINGLE_FRAME))
			return(0);
		// 4, 0 번째 index 불러온다.
		Load_Frame_BOB32(&npc[i], OBSTACLE_TEXTURE, 0, 0, 0, BITMAP_EXTRACT_MODE_CELL);
	...
	my_packet->size = strlen(id);
	send_wsabuf.len = BUF_SIZE;
	memcpy(my_packet->id, id, sizeof(id));
...
int Game_Main(void *parms)
{
	...
	for (int i = 0; i<20; ++i)
		for (int j = 0; j<20; ++j)
		{
			...
			if ((tile_x > BOARD_WIDTH - 1) || (tile_y > BOARD_HIGHT - 1)) continue;
	...
	for (int i = NPC_START ; i<MAX_NPC + MAX_OBSTACLE; ++i) 
		Draw_BOB32(&npc[i]);

	// draw some text
	wchar_t text[300];
	wsprintf(text, L"%S (%3d, %3d)\nhp : %d, mp: %d, Level : %d, exp : %d, sp : %d", 
		id ,player.x, player.y, playerHp, playerMp, playerLevel, playerExp, playerSp);
	Draw_Text_D3D(text, 10, screen_height - 94, D3DCOLOR_ARGB(255, 255, 255, 255));
